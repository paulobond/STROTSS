...
C = torch.from_numpy(
    np.float32([[0.577350, 0.577350, 0.577350], [-0.577350, 0.788675, -0.211325], [-0.577350, -0.211325, 0.788675]]))
...
def load_style_guidance(phi, path, coords_t, scale):
    style_im = to_device(
        Variable(load_path_for_pytorch(path, max_side=scale, verbose=False, force_scale=True).unsqueeze(0)))
    ...
    for i in range(len(zt)):
        ...
        temp = temp[:, :, s00, :].mul_(w00).add_(temp[:, :, s01, :].mul_(w01)).add_(temp[:, :, s10, :].mul_(w10)).add_(
            temp[:, :, s11, :].mul_(w11))
...
def load_style_folder(phi, paths, regions, ri, n_samps=-1, subsamps=-1, scale=-1, inner=1):
    ...
    for p in paths:
        ...
        style_im = to_device(
            Variable(load_path_for_pytorch(p, max_side=scale, verbose=False, force_scale=True).unsqueeze(0),
                     requires_grad=False))
