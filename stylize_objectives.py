...
class objective_class():
    ...
    def __init__(self, objective='remd_dp_g'):
    ...
    def gen_remd_dp_objective_guided(self, z_x, z_c, z_s, gz, d_ell, d_weight=10.0, content_weight=4.0,
        ...
        for ri in range(len(self.rand_ixx.keys())):
            ...
            ## Compute Style Loss ##
            remd_loss, used_style_feats = style_loss_func(x_st[:, :fm, :, :], z_st[:, :fm, :, :], self.z_dist,
                                                          splits=[fm])
            ...
            moment_ell += content_weight_frac * \
                          style_loss_func(x_st[:, :3, :, :], z_st[:, :3, :, :], self.z_dist, splits=[3])[0]
